...
from ...activations import SiLUActivation, gelu, gelu_new
...
from ..roberta.modeling_roberta import RobertaEmbeddings
from .configuration_xylent import XYLentConfig


logger = logging.get_logger(__name__)


XYLENT_PRETRAINED_MODEL_ARCHIVE_LIST = [
    "xylent-base",
    "xylent-large",
]
...
class XYLentSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in XYLentModel forward() function)
        ...
        if rel_pos is not None and not self.ignore_gru_gate:
            ...
            gate_u, gate_r = torch.sigmoid(
                self.gate_ur_linear(query_layer).view(_B, _H, _L, 2, 4).sum(-1, keepdim=False)
            ).chunk(2, dim=-1)
...
class XYLentAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = XYLentSelfAttention(config)
...
class XYLentLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.attention = XYLentAttention(config)
        self.is_decoder = config.is_decoder
        if self.is_decoder:
            self.crossattention = XYLentAttention(config)
    ...
    def forward(
        ...
        self_attention_outputs = self.attention(hidden_states, attention_mask, head_mask, rel_pos=rel_pos)
...
class XYLentEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.output_attentions = config.output_attentions
        self.output_hidden_states = config.output_hidden_states
        self.layer = nn.ModuleList([XYLentLayer(config, _) for _ in range(config.num_hidden_layers)])
...
class XYLentPreTrainedModel(PreTrainedModel):
    """An abstract class to handle weights initialization and
    a simple interface for downloading and loading pretrained models.
    """

    config_class = XYLentConfig
    base_model_prefix = "xylent"
...
class XYLentEmbeddings(nn.Module):
    def __init__(self, config):
        super().__init__()
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None):
        ...
        assert token_type_ids is None, "token_type_ids should be None for XYLentEmbeddings"
...
class XYLentModel(XYLentPreTrainedModel):
    def __init__(self, config):
        ...
        if self.config.initialize_token_type_embeddings:
            ...
            self.embeddings = XYLentEmbeddings(config)
        self.encoder = XYLentEncoder(config)
...
class XYLentForSequenceClassification(XYLentPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels

        self.xylent = XYLentModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class XYLentForTokenClassification(XYLentPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
...
class XYLentForQuestionAnswering(XYLentPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
